...
/**
 * Manejador global de excepciones con soporte internacionalizado (i18n).
 */
@ControllerAdvice
public class GlobalExceptionHandler {
	...
	public ResponseEntity<ApiError> handleValidationExceptions(MethodArgumentNotValidException ex, WebRequest request) {
		...
		String title = messageSource.getMessage("validation.error.title", null, locale);
		String message = messageSource.getMessage("validation.error.message", null, locale);

		ApiError error = ApiError.builder().timestamp(LocalDateTime.now()).status(HttpStatus.BAD_REQUEST.value())
				.error(title).message(message).details(details).path(request.getDescription(false)).build();

		return ResponseEntity.badRequest().body(error);
	}

	// Errores de validación JPA (persistencia)
	...
	public ResponseEntity<ApiError> handleRuntime(RuntimeException ex, WebRequest request) {
		Locale locale = LocaleContextHolder.getLocale();
		String title = messageSource.getMessage("business.error.title", null, locale);

		ApiError error = ApiError.builder().timestamp(LocalDateTime.now()).status(HttpStatus.BAD_REQUEST.value())
				.error(title).message(ex.getMessage()).path(request.getDescription(false)).build();
	...
	public ResponseEntity<ApiError> handleAllExceptions(Exception ex, WebRequest request) {
		Locale locale = LocaleContextHolder.getLocale();
		String title = messageSource.getMessage("server.error.title", null, locale);

		ApiError error = ApiError.builder().timestamp(LocalDateTime.now())
				.status(HttpStatus.INTERNAL_SERVER_ERROR.value()).error(title).message(ex.getMessage())
